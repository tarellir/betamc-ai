...
export function setupUI(player){
    ...
    // ============================
    const playerInventory = new Map(); // blockId -> count

    window.addToInventory = (blockId) => {
        const current = playerInventory.get(blockId) || 0;
        playerInventory.set(blockId, current + 1);
        updateHotbarDisplay();
    };

    window.removeFromInventory = (blockId) => {
        const current = playerInventory.get(blockId) || 0;
        if(current <= 0) return false;
        playerInventory.set(blockId, current - 1);
        if(current - 1 <= 0) playerInventory.delete(blockId);
        updateHotbarDisplay();
        return true;
    };

    window.hasBlockInInventory = (blockId) => {
        return (playerInventory.get(blockId) || 0) > 0;
    };

    window.getInventory = () => playerInventory;

    function updateHotbarDisplay() {
        // For each hotbar slot, show the block icon and count
        for(let i = 0; i < HOTBAR_COUNT; i++) {
            const bid = slots[i].blockId;
            const count = playerInventory.get(bid) || 0;
            // Update count display
            let countEl = slots[i].div.querySelector('.slot-count');
            if(count > 0) {
                if(!countEl) {
                    countEl = document.createElement('span');
                    countEl.className = 'slot-count';
                    slots[i].div.appendChild(countEl);
                }
                countEl.textContent = count > 1 ? count : '';
            } else if(countEl) {
                countEl.textContent = '';
            }
            // Dim the slot if count is 0
            if(count === 0) {
                slots[i].div.style.opacity = '0.4';
            } else {
                slots[i].div.style.opacity = '1';
            }
        }
    }

    // Expose select function for inventory clicks
    window.selectHotbarSlot = select;
}
