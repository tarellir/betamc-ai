...
import { BLOCKS, BLOCK_META } from "./world.js?v9";
...
export class Player {
    ...
    _interact(btn){
        ...
        if(btn === 0){
            ...
            if(oldType === BLOCKS.BEDROCK && this.gameMode !== 'creative') return; // can't break bedrock in survival
            if(oldType === BLOCKS.WATER) return;
            this.world.setBlock(bx, by, bz, 0);
            sfx.break(oldType);
            spawnBreakParticles(bx, by, bz, oldType);
            window.chatLog?.(`Bloc cassé: ${BLOCK_META[oldType]?.name || '#' + oldType}`, 'system');
        } else if(btn === 2){
            // Place
            const cam = this.camera.position;
            const inPlayer = Math.abs(nx+0.5 - cam.x) < HALF_W+0.5
                          && Math.abs(nz+0.5 - cam.z) < HALF_W+0.5
                          && ny < cam.y + 0.2
                          && ny + 1 > cam.y - PLAYER_H - 0.1;
            if(inPlayer) return;
            // Don't replace solid blocks
            const target = this.world.getBlock(bx, by, bz);
            if(target !== 0 && target !== BLOCKS.WATER && target !== -1) return;
